...
import org.onosproject.net.flow.criteria.PiCriterion; //NEW
import org.onosproject.net.pi.runtime.PiActionParam; //NEW
import org.onosproject.net.pi.model.PiActionParamId; //NEW
import org.onosproject.net.pi.model.PiMatchFieldId; //NEW
import org.onosproject.net.flow.DefaultTrafficSelector; //NEW
...
public class PipelinerImpl extends AbstractHandlerBehaviour implements Pipeliner {
    ...
    private static final String CLONE_TO_CPU = "IngressPipeImpl.clone_to_cpu";
		private static final String L2_TABLE = "IngressPipeImpl.l2_exact_table";
    private static final String SET_EP = "IngressPipeImpl.set_egress_port";
    ...
    private DeviceId deviceId;
		...
		private PortNumber p_num;
		private MacAddress mac_add;
...
// Create an equivalent FlowRule with same selector and set_egress_port action.
final PiAction setEgressPort = PiAction.builder()
				.withId(PiActionId.of(SET_EP))
				.withParameter(new PiActionParam(PiActionParamId.of("port_num"), p_num.toLong()))
				.build();


final FlowRule.Builder ruleBuilder2 = DefaultFlowRule.builder()
				.forTable(PiTableId.of(L2_TABLE))
				.forDevice(deviceId)
				.fromApp(obj.appId())
				.withPriority(obj.priority())
				//.withSelector(DefaultTrafficSelector.builder()
				//										 .matchPi(matchEthMac).build())
				.withTreatment(DefaultTrafficTreatment.builder()
				.piTableAction(setEgressPort).build());

final PiCriterion matchEthMac = PiCriterion.builder()
				.matchExact(PiMatchFieldId.of("hdr.ethernet.dst_addr"), mac_add.toBytes())
				.build();

				if (obj.permanent()) {
						ruleBuilder2.makePermanent();
				} else {
						ruleBuilder2.makeTemporary(obj.timeout());
				}
        ...
        switch (obj.op()) {
            case ADD:
                flowRuleService.applyFlowRules(ruleBuilder.build());
								//flowRuleService.applyFlowRules(ruleBuilder2.build());
                groupService.addGroup(cloneGroup);
                break;
            case REMOVE:
                flowRuleService.removeFlowRules(ruleBuilder.build());
								//flowRuleService.removeFlowRules(ruleBuilder2.build());
